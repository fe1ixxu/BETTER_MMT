    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        if not run_moe:
            ...
            l_aux = None
        elif run_moe and (not self.run_ed):
            x, fc_result, l_aux = self.forward_moe(
                x,
                tokens,
            )
        else:
            x, fc_result, l_aux = self.forward_moe_ed(
                x,
                tokens,
                self.fc3,
                self.activation_fn,
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        if not self.is_moe_layer or self.cfg.alternate_decoder_ffn_embed_dim > 0:
            ...
            l_aux = None
        elif self.is_moe_layer and (not self.run_ed):
            x, l_aux = self.forward_moe(x, tokens)
        else:
            x, l_aux = self.forward_moe_ed(x, tokens, self.fc3, self.activation_fn)
