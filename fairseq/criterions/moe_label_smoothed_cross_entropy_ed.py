...
@dataclass
class MoELabelSmoothedCrossEntropyEDCriterionConfig(MoECriterionConfig):
    ...
    ed_loss_weight: Optional[float] = field(
        default=1,
        metadata={
            "help": "Weight associated with ED loss in the weighted sum of ED"
        },
    )
...
@register_criterion(
    ...
    dataclass=MoELabelSmoothedCrossEntropyEDCriterionConfig,
)
class MoELabelSmoothedCrossEntropyEDCriterion(FairseqCriterion):
    moe_logging_keys = [
    ...
    def __init__(
        ...
        self.ed_loss_weight = ed_loss_weight
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss, moe_loss, cmr_loss, moe_metadata, ed_loss = self.compute_loss(
        ...
        logging_output = {
            ...
            "ed_loss": ed_loss.data * sample_size,
    ...
    def compute_loss(self, model, net_output, sample, sample_size, reduce=True):
        ...
        for l_ed_loss in net_output[1]["ed_loss"]:
            if l_ed_loss is not None:
                ed_loss += l_ed_loss
                ed_count += 1
        ...
        # ed_loss = sample_size * ed_loss
        loss = (
            ls_loss
            + self.gate_loss_weight * gate_loss
            + self.cmr_gate_loss_weight * cmr_gate_loss
            + self.ed_loss_weight * ed_loss
        )
        return loss, nll_loss, gate_loss, cmr_gate_loss, self.get_moe_metadata(model), ed_loss

    def get_moe_metadata(self, model):
        moe_logging_output = {}
        for key in MoELabelSmoothedCrossEntropyEDCriterion.moe_logging_keys:
            vals = []
            module_names = []
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        ed_loss_sum = sum(log.get("ed_loss", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "ed_loss", ed_loss_sum / sample_size, sample_size, round=8
        )
        ...
        all_keys = (
            MoELabelSmoothedCrossEntropyEDCriterion.moe_logging_keys
            + MoELabelSmoothedCrossEntropyEDCriterion.secondary_moe_logging_keys
        )
        for key in all_keys:
